...
import com.steam.util.OrderSource;

import javax.servlet.http.HttpServletRequest;


/**
 * Utils - 判断请求来自手机或电脑 <一句话功能简述> <功能详细描述>
 *
 * @author xutianlong
...
public class HttpRequestDeviceUtils {
     ...
     * @return 如果命中手机特征规则，则返回对应的特征字符串
     */
    public static OrderSource getSource(HttpServletRequest request) {
        OrderSource source = null;
        String deviceModel = request.getHeader("deviceModel");
        if (deviceModel != null && !deviceModel.trim().equals("")) {
            if (deviceModel.contains("iPod") || deviceModel.contains("iPhone")) {
                source = OrderSource.mobile;
            } else if (deviceModel.contains("iPad")) {
                source = OrderSource.ipad;
            } else {
                source = OrderSource.pc;
            }
        } else {
            source = OrderSource.pc;
        }
